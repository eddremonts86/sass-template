...
class PasswordValidation extends ValidationStrategy {
  ...
  validate(password) {
    ...
    if (!isValid) {
      const requirements = [];
      if (!hasMinLength) requirements.push(`at least ${this.minLength} characters`);
...
class DeleteTextCommand extends Command {
  ...
  execute() {
    ...
    const newText = text.substring(0, this.startIndex) + text.substring(this.endIndex);
    this.editor.setText(newText);
  }
  
  undo() {
    const text = this.editor.getText();
    const newText = text.substring(0, this.startIndex) + this.deletedText + text.substring(this.startIndex);
    this.editor.setText(newText);
  }
}
